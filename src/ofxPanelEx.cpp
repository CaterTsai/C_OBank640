...
ofImage ofxPanelEx::loadIcon;
ofImage ofxPanelEx::saveIcon;

ofxPanelEx::ofxPanelEx()
	:bGrabbed(false) {}

ofxPanelEx::ofxPanelEx(const ofParameterGroup & parameters, const std::string& filename, float x, float y)
...
ofxPanelEx * ofxPanelEx::setup(const std::string& collectionName, const std::string& filename, float x, float y) {
	if (!loadIcon.isAllocated() || !saveIcon.isAllocated()) {
		loadIcons();
	}
	registerMouseEvents();
	return (ofxPanelEx*)ofxGuiGroup::setup(collectionName, filename, x, y);
}

ofxPanelEx * ofxPanelEx::setup(const ofParameterGroup & parameters, const std::string& filename, float x, float y) {
	if (!loadIcon.isAllocated() || !saveIcon.isAllocated()) {
		loadIcons();
	}
	registerMouseEvents();
	return (ofxPanelEx*)ofxGuiGroup::setup(parameters, filename, x, y);
}

void ofxPanelEx::loadIcons() {
...
bool ofxPanelEx::mouseReleased(ofMouseEventArgs & args) {
	bool rVal = false;
	this->bGrabbed = false;

	if (ofxGuiGroup::mouseReleased(args)) {
		ofNotifyEvent(isUpdateE, this);
		return true;
	}
...
bool ofxPanelEx::setValue(float mx, float my, bool bCheck) {
